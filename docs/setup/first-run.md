# Первый запуск

## 1. Установи mise

mise — менеджер версий инструментов. Он установит всё остальное.

```bash
curl https://mise.run | sh
```

Затем добавь активацию в конфиг своего shell.

**Bash** (`~/.bashrc`):
```bash
eval "$(~/.local/bin/mise activate bash)"
```

**Zsh** (`~/.zshrc`):
```zsh
eval "$(~/.local/bin/mise activate zsh)"
```

Перезапусти терминал или выполни `source ~/.bashrc` / `source ~/.zshrc`.

---

## 2. Клонируй репозиторий

```bash
git clone <repo-url>
cd ingot
```

---

## 3. Установи инструменты проекта

```bash
mise install
```

mise прочитает `mise.toml` и установит все нужные версии инструментов — age, fnox и остальное. После этого они будут доступны в этой папке автоматически.

Проверь что всё встало:

```bash
mise current
```

Должны появиться все инструменты из `mise.toml` с нужными версиями.

---

Дальше — [генерация ключа и получение доступа к секретам](./secrets-access.md).
